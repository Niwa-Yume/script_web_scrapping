...
for event in events:
    ...
    event_dateraw = soup.h3.text
    event_date = dateparser.parse(event_dateraw.replace(event_dateraw.split(" ")[0], "")).date()
    ...
    if event_pratique_text:
        ...
        event_time = event_pratique_text.strip()[:5]
...
# Ajout de la date de l'événement masqué (date d'exécution du script)
df["event_maskedlocationdate"] = str(current_datetime).split(" ")[0]

# Ajout de la date de l'événement masqué (date d'exécution du script)
df["event_maskedlocationtime"] = str(current_datetime).split(" ")[1].split(".")[0]

# Ajout du prix de l'entrée
df["event_price"] = 0

#réorganisation de l'ordre
df = df[["event_id", "event_title", "event_date", "event_time","event_location", "event_lat", "event_lon", "event_description", "event_type", "event_category", "event_private", "event_maskedlocationdate", "event_maskedlocationtime", "event_price", "event_creation", "event_user_id", "event_imageevent_id"]]
